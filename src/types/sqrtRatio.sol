...
function toFixed(SqrtRatio sqrtRatio) pure returns (uint256 r) {
    assembly ("memory-safe") {
        // Determine which region based on prefix bits (95-94)
        let prefix := shr(94, sqrtRatio)
        
        // Calculate shift amount: 2 + prefix * 32
        let shiftAmount := add(2, shl(5, prefix))
        
        // Remove the prefix marker bit(s) and shift left to restore original scale
        // Region markers: 00=none, 01=bit94, 10=bit95, 11=both
        let markerMask
        switch prefix
        case 0 { markerMask := 0 }  // 00: no marker bits
        case 1 { markerMask := TWO_POW_94 }  // 01: bit 94
        case 2 { markerMask := TWO_POW_95 }  // 10: bit 95
        case 3 { markerMask := or(TWO_POW_95, TWO_POW_94) }  // 11: both
        
        r := shl(shiftAmount, sub(sqrtRatio, markerMask))
    }
}
