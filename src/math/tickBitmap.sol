    ...
    returns (int32 nextTick, bool isInitialized)
{
    unchecked {
        ...
        while (true) {
            ...
            (uint8 nextIdx, bool found) = bitmap.nextBelow(uint8(index));
            uint256 nextIndex = found ? uint256(nextIdx) : 0;
    ...
    returns (int32 prevTick, bool isInitialized)
{
    unchecked {
        ...
        while (true) {
            ...
            // find the index of the previous tick in that word
            (uint8 prevIdx, bool foundPrev) = bitmap.nextAbove(uint8(index));
            uint256 prevIndex = foundPrev ? uint256(prevIdx) : 0;
