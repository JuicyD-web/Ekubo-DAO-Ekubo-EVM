...
contract BitmapTest is Test {
    ...
    function test_leSetBit(Bitmap bitmap, uint8 index) public pure {
        (uint8 prevIdx, bool foundPrev) = bitmap.leSetBit(index);
        uint256 prev = foundPrev ? uint256(prevIdx) : type(uint256).max;
        if (prev != 0) {
    ...
    function test_geSetBit(Bitmap bitmap, uint8 index) public pure {
        (uint8 nextIdx, bool foundNext) = bitmap.geSetBit(index);
        uint256 next = foundNext ? uint256(nextIdx) : type(uint256).max;
